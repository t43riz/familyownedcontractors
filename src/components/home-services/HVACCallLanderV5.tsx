...
export default function HVACCallLanderV5() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col overflow-x-hidden bg-slate-100 sm:justify-center">
      {/* Animated USA-colors outlining stroke — a traveling red/white/blue
          ring + gentle glow that draws low-tech users to the active control
          (the ZIP field, then the call CTA). Scoped to this variation. */}
      <style>{`
        @property --foc-usa-angle { syntax: "<angle>"; initial-value: 0deg; inherits: false; }
        @keyframes foc-usa-spin { to { --foc-usa-angle: 360deg; } }
        @keyframes foc-usa-pulse {
          0%, 100% { box-shadow: 0 0 0 0 rgba(0,51,160,0); }
          50%      { box-shadow: 0 0 13px 2px rgba(0,51,160,0.35); }
        }
        .foc-usa-stroke {
          position: relative;
          background: conic-gradient(from var(--foc-usa-angle),
            #C8102E, #ffffff, #0033A0, #ffffff, #C8102E);
          animation: foc-usa-spin 2.6s linear infinite, foc-usa-pulse 2s ease-in-out infinite;
        }
        .foc-usa-stroke--cta {
          background: conic-gradient(from var(--foc-usa-angle),
            #C8102E, #ffffff, #0033A0, #ffffff, #C8102E);
          animation: foc-usa-spin 2.2s linear infinite, foc-usa-pulse 1.7s ease-in-out infinite;
        }
        @media (prefers-reduced-motion: reduce) {
          .foc-usa-stroke, .foc-usa-stroke--cta { animation: none; }
        }
      `}</style>
      ...
      <div className="mx-auto flex w-full max-w-md flex-col bg-white shadow-xl sm:my-4 sm:rounded-2xl sm:overflow-hidden">
        ...
        >
          {/* ZIP field wrapped in the animated USA stroke while awaiting input;
              the 3px padding is the visible ring. Calms down once submitted. */}
          <div className={`min-w-0 flex-1 rounded-full p-[3px] ${submitted ? '' : 'foc-usa-stroke'}`}>
            <input
              ref={inputRef}
              type="text"
              inputMode="numeric"
              ...
              className="w-full rounded-full bg-[#f0f2f5] px-4 py-2.5 text-base text-gray-900 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-[#0084ff]/40 disabled:opacity-60"
            />
          </div>
          <button
            type="submit"
